% In this file, we setup a uniform rectangular grid inside Cerebrum.
function Setup_Cartesian_Grid(NumPntsGrid,headmodelname)
    %Inputs NumPntsGrid in x, y, z direction. 
    %Output is the headmodel, vertices, triangles and nodes and it will
    %be saved in the headmodel name.
    cvec=[-0.0043    0.0169    0.0672]; % Center of the Head Model.
    fprintf('\n');
    fprintf('Setting Up Cartesian Grid....Please Wait !!!\n');
    fprintf('\n');
    N=NumPntsGrid;
    filename='brain.tri';
    [vertices,tri]=read_tri_files(filename);
    tri=tri+1;
    ...
    for idx=1:N
        ...
        for jdx=1:N
            ...
            for kdx=1:N
                z=zvec(kdx);
                testp=[x y z];
                in = intriangulation(vertices,tri,testp);
                if (in==1)
                    cnt=cnt+1; 
                    nodes(cnt,:)=testp;
                end
    ...
    hold on;
    plot3(nodes(:,1),nodes(:,2),nodes(:,3),'rx');
    headmodel.cartesian_grid=nodes;
    headmodel.vertices=vertices;
    headmodel.tri=tri;
    headmodel.center=cvec;
    save(headmodelname,'headmodel');
